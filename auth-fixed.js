...
// ── Shared / Relationship data helpers (NEW for paired features) ──────────────

/**
 * Returns the current user's active relationship ID (from profile), or null.
 */
export async function getMyRelationshipId() {
  const profile = await getMyProfile();
  return profile?.relationshipId || null;
}

/**
 * Returns a Firestore collection reference for the shared notes
 * of the current active relationship.
 * Throws if no active relationship.
 * Usage: sharedNotesCol() → collection(db, 'relationships', relId, 'notes')
 */
export async function sharedNotesCol() {
  const relId = await getMyRelationshipId();
  if (!relId) throw new Error('No active relationship. Pair your accounts first on the Pair page.');
  return collection(db, 'relationships', relId, 'notes');
}

/**
 * Returns the full relationship object for the current user (with .id).
 */
